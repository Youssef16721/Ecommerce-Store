...
function sendUserEmail($admin_email, $subject, $message) {
...
if (isset($_POST['send_email'])) {
    ...
    $admin_password = randomPassword(); // in common_functions file
    $admin_password_hashed = password_hash($admin_password, PASSWORD_DEFAULT);
    $update_pass = "UPDATE `admin` SET `admin_password` = ? WHERE `admin_name` = ? AND `admin_email` = ?";
    $update_pass_statement = mysqli_prepare($conn, $update_pass);
    mysqli_stmt_bind_param($update_pass_statement, "sss", $admin_password_hashed, $admin_name, $admin_email);
    if (mysqli_stmt_execute($update_pass_statement)){
    $subject = "Your new password";
    $message = nl2br("Hello Admin $admin_name,\n\nYour password is: $admin_password

    Please save this username because you will need it to login. And DO NOT SHARE IT with anyone.");

    $mail_sent = sendUserEmail($admin_email, $subject, $message);
    ...
    $message = nl2br("Hello Admin {$admin_name}, your password is {$admin_password}.
            ...
            <div class="col-lg-12 col-xl-6">
                <form action="" method="post">
                    <div class="form-outline mb-4">
                        <h5 class="text-center">How would you like to receive your Password?</h5>
                    </div>
                    ...
                    <?php elseif(isset($_POST['send_sms'])):?>
                        <?php if($sms_sent):?>
                    <div class="alert alert-success" role="alert">
                        <h4 class="alert-heading">Done!</h4>
                    ...
                    <div class="mt-4 pt-2 d-flex justify-content-center">
                        <a href="verify.php" class="btn btn-secondary">Back</a>
                    </div>
                </form>
            </div>
        </div>
